...
appVar.controller("ctrl1", function($scope, $http) {
	...
	angular.forEach($scope.tuitionjson, function(item) {
		//console.log(item);
		console.log("Items2 list "+JSON.stringify(item));
...
appVar.controller("baysianCtrl", function($scope,$http) {
					...
					$http({
						...
						console.log("Output is"+JSON.stringify($scope.baysian));
					...
					$scope.calculate = function(tuition, earning, comp) {
						...
						angular.forEach($scope.baysian, function(item) {
							...
							if (item.MALE_RPY == "Yes"
									& item.TUITIONFEE_IN == tuition) {
								$scope.tuition = $scope.tuition + 1;
								$scope.count = $scope.count + 1;
							}
							if (item.MALE_RPY == "Yes"
									& item.EARN_2011 == earning) {
						...
						$scope.prob = ($scope.tuition / $scope.total)
								* ($scope.earnings / $scope.total)
								* ($scope.comp / $scope.total);
						$scope.baysiandata.push({
							'fees' : tuition,
							'earn' : earning,
							'comp' : comp,
							'prob' : $scope.prob,
							'college' : angular.copy($scope.college)
						})
...
//		{
//
//			$scope.yearly = data.schools[0].array;
//
//			console.log("Output is" + JSON.stringify($scope.tuitionjson));
//
//			console.log("Success");
